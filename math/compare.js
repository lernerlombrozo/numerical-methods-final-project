function makeComparissonChart(x0,xf,h){
    ...
    const cSimp = compositeSimpson(x0,xf,h)
    const cSimp38 = compositeSimpson38(x0,xf,h)
    const GQ2 = gaussianQuadrature(x0,xf,2)
    const GQ3 = gaussianQuadrature(x0,xf,3)
    const table = [
        ['Method',`integral from ${x0} to ${xf}`,'error'],
        [`Reference ${analyticalSolution? 'analytical solution': '(composite n=2 h=0.00001)' }`, ref,null],
